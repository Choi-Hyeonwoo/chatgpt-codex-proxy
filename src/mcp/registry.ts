  ...
  */
  async initialize(): Promise<void> {
    ...
    if (enabled.length === 0) {
      ...
      if (!envVal) {
        log.info("disabled (PROXY_MCP_SERVERS not set)");
      } else {
        log.warn(`no matching servers for PROXY_MCP_SERVERS="${envVal}"`);
      }
      return;
    }

    log.info(`connecting to: ${enabled.join(", ")}`);
    ...
    for (const result of results) {
      if (result.status === "rejected") {
        log.error("server load failed", result.reason);
        continue;
      }
      const { name, tools } = result.value;
      log.info(`${name}: ${tools.length} tools loaded`);
    ...
    log.info(`ready: ${this.tools.length} total MCP tools`);
  }
